...
export const JournalView: React.FC<JournalViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      {deletedBackup && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-4 py-3 rounded-2xl shadow-2xl text-xs font-semibold bg-slate-900 border border-slate-700 text-slate-100 flex items-center gap-3 backdrop-blur-md animate-fade-in">
          <span>Deleted {deletedBackup.length} entry{deletedBackup.length > 1 ? 's' : ''}</span>
          <button
            onClick={handleUndoDelete}
            className="flex items-center gap-1 px-3 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white shadow font-bold"
          >
      ...
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/90 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xl">
      ...
      {/* Search & Filter Bar */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-900/60 border border-slate-800/80 rounded-2xl p-3">
        <div className="sm:col-span-2 relative">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          />
        </div>
            ...
            return (
              <div
                ...
                className={`bg-slate-900/90 border rounded-2xl overflow-hidden shadow-md transition-all ${
                  isSelected ? 'border-indigo-500/70 bg-slate-900' : 'border-slate-800 hover:border-slate-700'
                }`}
              >
                {/* Transaction Summary Header */}
                <div
                  onClick={() => toggleExpand(tx.id)}
                  className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 cursor-pointer hover:bg-slate-800/40 transition-colors"
                >
                  <div className="flex items-start sm:items-center gap-2.5 min-w-0">
                    ...
                    <button
                      ...
                      className="mt-0.5 sm:mt-0 p-1 rounded hover:bg-slate-800"
                    >
                      {isSelected ? (
                        <CheckSquare className="w-4 h-4 text-indigo-400" />
                      ) : (
                    ...
                    <button
                      type="button"
                      className="mt-0.5 sm:mt-0 p-1 rounded bg-slate-800 text-slate-400 hover:text-white"
                    >
                      {isExpanded ? (
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono text-[10px] font-semibold text-slate-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">
                          {tx.date}
                        </span>
      ...
      {selectedTxIds.size > 0 && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-slate-900/95 border border-slate-700 rounded-2xl shadow-2xl px-4 py-2.5 flex items-center gap-3 backdrop-blur-md animate-fade-in">
          <span className="text-xs font-bold text-white whitespace-nowrap">
            {selectedTxIds.size} Selected
          </span>
          <button
            onClick={() => setSelectedTxIds(new Set())}
            className="px-2.5 py-1 text-xs text-slate-300 hover:text-white bg-slate-800 rounded-lg"
          >
            Deselect
          </button>
          <button
            onClick={promptDeleteSelected}
            className="px-3.5 py-1 text-xs font-bold text-white bg-rose-600 hover:bg-rose-500 rounded-lg shadow-md flex items-center gap-1.5 transition-all active:scale-95"
          >
            <Trash2 className="w-3.5 h-3.5" />
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 max-w-sm w-full shadow-2xl space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-1.5 text-xs font-bold text-white bg-rose-600 hover:bg-rose-500 rounded-lg shadow transition-all active:scale-95"
              >
                Delete
              </button>
            </div>
